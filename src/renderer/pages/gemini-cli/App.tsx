    ...
    <div className="flex h-screen flex-col bg-background text-foreground">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-medium">Gemini CLI</h1>
        ...
        {messages.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 py-12">
            ...
            <div className="text-center">
              <h2 className="text-lg font-medium">Gemini CLI</h2>
              <p className="mt-1 text-sm text-muted-foreground">Start a conversation with Gemini</p>
            </div>
          </div>
        ) : (
          messages.map((msg) => (
            <Message
              key={msg.id}
              message={msg}
              avatar={msg.role === 'assistant' ? <GeminiIcon size={16} /> : undefined}
            />
          ))
        )}
      </ChatContainer>
      ...
      <PromptInput
        ...
        placeholder={isConnected ? 'Ask Gemini anything...' : 'Connecting to Gemini...'}
      />
    </div>
  )
}
