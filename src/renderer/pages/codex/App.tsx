  ...
  return (
    <div className="flex h-screen flex-col bg-background text-foreground">
        ...
        {messages.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 py-12">
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Start a conversation with Codex</p>
            </div>
          </div>
        ) : (
          messages.map((msg) => (
            <Message
              key={msg.id}
              message={msg}
              avatar={msg.role === 'assistant' ? <OpenAIIcon size={16} /> : undefined}
            />
          ))
        )}
      </ChatContainer>
      ...
      <PromptInput
        ...
        placeholder={isConnected ? 'Ask Codex anything...' : 'Connecting to Codex...'}
      />
    </div>
  )
}
